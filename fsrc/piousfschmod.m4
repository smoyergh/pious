/*
 * pious_schmod() Fortran interface - See src/plib/plib.h for description.
 *
 * @(#)piousfschmod.m4	2.2  28 Apr 1995  Moyer
 */
...
void
FUNCTION(piousfschmod) ARGS(`STRING_ARG(hname),
                             STRING_ARG(spath),
                             STRING_ARG(lpath),
                             dsvcnt,
                             STRING_ARG(path),
  ...
  int *dsvcnt, *mode, *rc;
{
  ...
  /* fill dsv structure and make call. */
  ...
  else
    {
      ...
      *rc = pious_schmod(dsv, *dsvcnt, pbuf, (pious_modet)(*mode));
    }
