/*
 * pious_smkdir() Fortran interface - See src/plib/plib.h for description.
 *
 * @(#)piousfsmkdir.m4	2.2  28 Apr 1995  Moyer
 */
...
void
FUNCTION(piousfsmkdir) ARGS(`STRING_ARG(hname),
                             STRING_ARG(spath),
                             STRING_ARG(lpath),
                             dsvcnt,
                             STRING_ARG(path),
  ...
  int *dsvcnt, *mode, *rc;
{
  ...
  /* fill dsv structure and make call. */
  ...
  else
    {
      ...
      *rc = pious_smkdir(dsv, *dsvcnt, pbuf, (pious_modet)(*mode));
    }
