/*
 * pious_sopen() Fortran interface - See src/plib/plib.h for description.
 *
 * @(#)piousfsopen.m4	2.2  28 Apr 1995  Moyer
 */
...
void
FUNCTION(piousfsopen) ARGS(`STRING_ARG(hname),
                            STRING_ARG(spath),
                            STRING_ARG(lpath),
                            dsvcnt,
                            STRING_ARG(path),
  ...
  int *dsvcnt, *oflag, *mode, *rc;
{
  ...
  /* fill dsv structure and make call. */
  ...
  else
    {
      ...
      *rc = pious_sopen(dsv, *dsvcnt, pbuf, *oflag, (pious_modet)(*mode));
    }
