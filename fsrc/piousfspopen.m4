/*
 * pious_spopen() Fortran interface - See src/plib/plib.h for description.
 *
 * @(#)piousfspopen.m4	2.2  28 Apr 1995  Moyer
 */
...
void
FUNCTION(piousfspopen) ARGS(`STRING_ARG(hname),
                             STRING_ARG(spath),
                             STRING_ARG(lpath),
                             dsvcnt,
                             STRING_ARG(group),
  ...
  int *dsvcnt, *view, *map, *faultmode, *oflag, *mode, *seg, *rc;
{
  ...
  if (*dsvcnt <= 0 ||
      ...
      STRING_LEN(lpath) < (PIOUS_PATH_MAX * (*dsvcnt)))
    {
      *rc = PIOUS_EINVAL;
           ...
           !f2cstr(gbuf, PIOUS_NAME_MAX, STRING_PTR(group), STRING_LEN(group)))
    {
