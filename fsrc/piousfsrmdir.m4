/*
 * pious_srmdir() Fortran interface - See src/plib/plib.h for description.
 *
 * @(#)piousfsrmdir.m4	2.2  28 Apr 1995  Moyer
 */
...
void
FUNCTION(piousfsrmdir) ARGS(`STRING_ARG(hname),
                             STRING_ARG(spath),
                             STRING_ARG(lpath),
                             dsvcnt,
                             STRING_ARG(path),
  ...
  int *dsvcnt, *rc;
{
  ...
  /* fill dsv structure and make call. */
  ...
  else
    {
      ...
      *rc = pious_srmdir(dsv, *dsvcnt, pbuf);
    }
